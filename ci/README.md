# CI: the vgi-cve worker integration suite
...
1. **Build the worker** — `go build -o vgi-cve-worker ./cmd/vgi-cve-worker`.
   The resulting binary is a self-contained stdio worker the extension can
   spawn; `VGI_CVE_WORKER` (an absolute path) is the ATTACH `LOCATION`.
...
4. **Run** — [`run-integration.sh`](run-integration.sh) builds + starts the
   repo's `mockserver` (a canned NVD 2.0 API) on a free port (exporting
   `VGI_CVE_TEST_URL`), stages the preprocessed tree, points `VGI_CVE_WORKER` at
   the built worker binary, warms the extension cache once (`INSTALL vgi FROM
   community`), then runs the suite in a single `haybarn-unittest` invocation.
   Any failed assertion exits non-zero and fails the job. The mock server is
   killed on exit.

## Run it locally

```bash
go build -o vgi-cve-worker ./cmd/vgi-cve-worker
# point HAYBARN_UNITTEST at a haybarn-unittest binary (or a local DuckDB
# `unittest` built with the vgi extension):
HAYBARN_UNITTEST=/path/to/haybarn-unittest \
VGI_CVE_WORKER="$PWD/vgi-cve-worker" \
  ci/run-integration.sh
```

Or use the Makefile target (`make test-sql`), which builds both binaries,
starts the mock NVD server, and points the worker at `$(CURDIR)/vgi-cve-worker`
